...
PROMPT = (
    ...
    "回答は登場人物や世界観の設定、アイテム名などと、その要素ごとの情報で整理し、箇条書きの形式で記述して提供してください。"
)

rag = create_rag_service_from_env()

@function_tool
async def setting_search(query: str) -> str:
    """
    このツールは小説のシーンの適切な描写に必要な登場人物や世界観、その他アイテムなどの設定をまとめたデータへの検索機能を提供します。
    このツールを利用する際は、検索クエリに可能な限り多くのコンテキスト(例えば登場人物名、アイテム名など)を含め、自然言語でクエリを記述してください。
